...
def train(env, args, writer): 
    current_model = Rainbow(env, args).to(args.device)
    target_model = Rainbow(env, args).to(args.device)
    opponent = SimpleAi(env, player_id=2)
    ...
    for frame_idx in range(1, args.max_frames + 1):
        ...
        op_action = opponent.get_action(torch.FloatTensor(state).to(args.device))
        next_state, (reward, op_reward), done, _ = env.step((action, op_action))
