...
blank_trans = Transition(0, torch.zeros(50, 50, dtype=torch.uint8), None, 0, False)
...
class PrioritizedReplayMemory():
    ...
    def append(self, state, action, reward, terminal):
        ...
        last_state = torch.tensor(state[-1])
        state = last_state.mul(255).to(dtype=torch.uint8, device=torch.device('cpu'))
